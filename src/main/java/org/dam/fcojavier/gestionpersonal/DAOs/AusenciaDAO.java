...
public class AusenciaDAO implements CrudDAO<Ausencia> {
    private final String insert_SQL = "INSERT INTO ausencia (motivo) VALUES (?)";
    private final String update_SQL = "UPDATE ausencia SET motivo = ? WHERE id_ausencia = ?";
        ...
        if(ausencia!=null) {
            ...
            if(ausenciaExistente!=null) {
                try(PreparedStatement pstm = ConnectionDB.getConnection().prepareStatement(update_SQL)) {
                    pstm.setString(1, ausencia.getMotivo());
                    pstm.setInt(2, ausenciaExistente.getIdAusencia());
                    if(pstm.executeUpdate() > 0) {
